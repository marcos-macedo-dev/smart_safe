<template>
  <div class="container mx-auto py-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
      <div class="bg-white dark:bg-zinc-800 rounded-md shadow-sm p-4 border border-zinc-200 dark:border-zinc-700">
        <div class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Total</div>
        <div class="text-2xl font-bold text-zinc-900 dark:text-white mt-1">{{ stats.total }}</div>
      </div>
      
      <div class="bg-white dark:bg-zinc-800 rounded-md shadow-sm p-4 border border-zinc-200 dark:border-zinc-700">
        <div class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Pendentes</div>
        <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-400 mt-1">{{ stats.pendente }}</div>
      </div>
      
      <div class="bg-white dark:bg-zinc-800 rounded-md shadow-sm p-4 border border-zinc-200 dark:border-zinc-700">
        <div class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Ativos</div>
        <div class="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">{{ stats.ativo }}</div>
      </div>
      
      <div class="bg-white dark:bg-zinc-800 rounded-md shadow-sm p-4 border border-zinc-200 dark:border-zinc-700">
        <div class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Fechados</div>
        <div class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{{ stats.fechado }}</div>
      </div>
      
      <div class="bg-white dark:bg-zinc-800 rounded-md shadow-sm p-4 border border-zinc-200 dark:border-zinc-700">
        <div class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Cancelados</div>
        <div class="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">{{ stats.cancelado }}</div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-md shadow-sm overflow-hidden">
      <!-- Cabeçalho da tabela -->
      <div class="hidden md:grid grid-cols-12 gap-4 px-4 py-3 bg-zinc-50 dark:bg-zinc-700/30 border-b border-zinc-200 dark:border-zinc-700 text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">
        ...
        >
          ...
          <div class="hidden md:grid grid-cols-12 gap-4 items-center">
            ...
            <div class="col-span-3">
              ...
              <div class="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                {{ sos.latitude && sos.longitude ? `${sos.latitude}, ${sos.longitude}` : 'Localização indisponível' }}
              </div>
            </div>
...
const applyFilters = () => {
  ...
  if (filters.value.search) {
    ...
    filtered = filtered.filter(sos => 
      ...
      (sos.usuario?.nome_completo && sos.usuario.nome_completo.toLowerCase().includes(searchTerm)) ||
...
const getStatusClass = (status) => {
  const classMap = {
    ...
    'aguardando_autoridade': 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
